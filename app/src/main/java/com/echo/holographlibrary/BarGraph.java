        ...
        if (fullImage == null || shouldUpdate) {
            ...
            for (Bar p : points) {
                ...
                if (showBarText) {
                    ...
                    this.p.getTextBounds(unit + Util.formatPrice(p.getValue()), 0, 1, r2);
                    //if (popup != null)
                    //    popup.setBounds((int) (((r.left + r.right) / 2) - (this.p.measureText(unit + Util.formatPrice(p.getValue())) / 2)) - 14, r.top + (r2.top - r2.bottom) - 26, (int) (((r.left + r.right) / 2) + (this.p.measureText(unit + Util.formatPrice(p.getValue())) / 2)) + 14, r.top);
                    popup.draw(canvas);
                    if (isAppended())
                        canvas.drawText(p.getValue() + unit, (int) (((r.left + r.right) / 2) - (this.p.measureText(unit + p.getValue()) / 2)), r.top - 20, this.p);
                    else
                        canvas.drawText(unit + Util.formatPrice(p.getValue()), (int) (((r.left + r.right) / 2) - (this.p.measureText(unit + Util.formatPrice(p.getValue())) / 2)), r.top - 20, this.p);
                }
